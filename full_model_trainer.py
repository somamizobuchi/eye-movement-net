...
class FullModelTrainer:
    ...
    def train_step(self, batch):
        ...
        # Regularize temporal convolutions to encourage smooth temporal filters
        temporal_smoothness = (
            self.model.encoder.temporal_kernels.diff(dim=1).square().mean(dim=1).mean()
        )
        ...
        kernel_variance_loss = (
            self.kernel_variance_weight * self.model.kernel_variance()
        )
