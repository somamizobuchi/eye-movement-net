...
from recon_model import ReconsModel
from utils import (
    rescale,
)
...
class TrainingConfig:
    ...
    sigma: float = 1e-6  # Spatial jerk energy (smoothness)
    gamma: float = 1e-7  # Regularization
    theta: float = 1e-4  # Temporal filter regularization
...
class Trainer:
    ...
    def setup_model(self):
        ...
        self.dataset = ReconDataset(
            img_size=256,
            roi_size=self.config.kernel_size,
            total_samples=self.config.drift_samples,
            sampling_frequency=self.config.fs,
            diffusion_coefficient=20 / 3600,
            pixels_per_degree=240,
            pad_start=self.config.kernel_length - 1,
        )
    ...
    def log_metrics(self):
        ...
        with torch.no_grad():
            ...
            self.writer.add_image(
                ...
                rescale(
                    torch.cat(
                        [self.current_target[0], self.current_reconstruction[0]], dim=1
                    )
                ),
                self.iteration,
                dataformats="HW",
