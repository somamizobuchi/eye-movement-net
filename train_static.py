...
import numpy as np
from utils import apply_roi_mask, accumulate_frames_at_positions, normalize_unit_variance, rescale
...
def main():
    ...
    fs = 480.
    ppd = 180
    diffusion_constant = 30. / 3600.  # deg^2/sec
    drift_samples = 256
    delay_samples = 1
    ...
    tq_range = trange(iteration, iteration + iterations,
    ...
    for i in tq_range:
        ...
        target = torch.stack([apply_roi_mask(image[b, :],
                                             eye_px[b, model.kernel_length-1:, :],
                                             (model.kernel_size,
                                              model.kernel_size),
                                             crop_center=True) for b in range(image.shape[0])])

        recons = torch.stack([accumulate_frames_at_positions(
            image[0].shape,
            out[b, :],
            eye_px[b, model.kernel_length-1:, :],
            True) for b in range(image.shape[0])]) / out.shape[1]

        # Compute the loss and its gradients
        loss_mse = torch.nn.functional.mse_loss(recons, target)
        loss_jerk = alpha * model.jerk_energy_loss()
        loss_fr = beta * (model.spatial_kernels.square().mean() +
        ...
        if i % log_iterations == log_iterations-1:
            ...
            with torch.no_grad():
                ...
                writer.add_images("Reconstruction/Spatial", rescale(torch.concat(
                    (target, recons), 0))[:, None, :, :], i, dataformats='NCHW')
                ...
                # Log loss metrics
                writer.add_scalar(
                    'Loss/train', running_loss / log_iterations, i)
                writer.add_scalar('Loss/mse', running_mse / log_iterations, i)
                writer.add_scalar(
                    'Loss/jerk', running_jerk / log_iterations, i)
                writer.add_scalar(
                    'Loss/firing', running_fr / log_iterations, i)
