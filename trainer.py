...
class Trainer:
        ...
        self.writer.add_hparams(
            {
                ...
                "spatial_var_weight": spatial_var_weight,
            },
            {
                "reconstruction_loss": 0.0,
                "l2_loss": 0.0,
                "spatial_var_loss": 0.0,
            },
        )
    ...
    def train_step(self, batch):
        ...
        if self.reconstruct:
            err = torch.tensor(0.0, device=self.device)
            for i in range(output.shape[0]):
                reconstruction = self.dataloader.dataset.reconstruct_static_image(
                    batch[2][i, :, self.encoder.T - 1 :],
                    output[i].cpu(),
                    batch[1].shape[1],
                )
                err += (
                    (batch[1][i][batch[3][i]] - reconstruction[batch[3][i]])
                    .abs()
                    .mean()
                )
    ...
    def log_to_tensorboard(self, loss, iteration):
        ...
        if self.reconstruct:
            self.writer.add_image(
                ...
                torch.cat(
                    (
                        self.rescale(self.current_target),
                        self.rescale(self.current_output),
                    ),
                    dim=1,
                ),
            ...
            video = torch.cat(
                [
                    self.current_target[0].unsqueeze(0),
                    self.current_output[0].unsqueeze(0),
                ],
                dim=0,
            )
