...
class Encoder(nn.Module):
    def __init__(self,
        ...
        self.spatial_kernels = torch.nn.Parameter(
            torch.full([self.kernel_size**2, self.n_kernels], 0.))
        self.temporal_kernels = torch.nn.Parameter(torch.full(
            [n_kernels, self.kernel_length - self.temporal_delay], 0.))
    ...
    def temporal_kernels_full(self) -> torch.Tensor:
        return torch.concat((self.temporal_kernels, torch.zeros([self.n_kernels, self.temporal_delay], dtype=torch.float)), dim=1)
